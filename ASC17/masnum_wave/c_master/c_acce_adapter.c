        ...
        )
{
    ...
    cpe_param.host_flag  = &host_flag[0];
    cpe_param.slave_flag  = &slave_flag[0];
    int i;
...
/*
    ...
    host_flag[PACKED_PTR] = ppg_packed_data + (offset * PPG_SV_DATA);
    host_flag[OUT_DATA_PTR] = pexx_output_buf + (offset * NUM_PEXX);
    host_flag[IAS_PTR] = ppg_ia_set + offset;
    host_flag[ICS_PTR] = ppg_ic_set + offset;
    host_flag[NSPS_PTR] = ppg_nsp_set + offset;
    host_flag[TOTAL_NSP_PTR] = _nsp;
    asm volatile ("#nop":::"memory");
