...
void cpe_c_snonlin()
{
    ...
    float se_write_buf[4][_jl];
	float dse_write_buf[4][_jl];
    ...
    for (k = 1; k <= _kl; k++)
    {
        ...
        for (mr = 1; mr <= 2; mr++)
        {
            for (j = 1; j <= _jl; j+=4)
            {
                ...
                ans[0][0] = two_fcen * ad;    simd_store(ans[0][0],&_ans[0][0]);
                ans[0][1] = two_fcen * delad; simd_store(ans[0][1],&_ans[0][1]);

                ans[1][0] = adp * wp11;       simd_store(ans[1][0],&_ans[1][0]);
                ans[1][1] = deladp * wp112;   simd_store(ans[1][1],&_ans[1][1]);

                ans[2][0] = adp * wp12;       simd_store(ans[2][0],&_ans[2][0]);
                ans[2][1] = deladp * wp122;   simd_store(ans[2][1],&_ans[2][1]);

                ans[3][0] = adp * wp21;       simd_store(ans[3][0],&_ans[3][0]);
                ans[3][1] = deladp * wp212;   simd_store(ans[3][1],&_ans[3][1]);

                ans[4][0] = adp * wp22;       simd_store(ans[4][0],&_ans[4][0]);
                ans[4][1] = deladp * wp222;   simd_store(ans[4][1],&_ans[4][1]);

                ans[5][0] = adm * wm11;       simd_store(ans[5][0],&_ans[5][0]);
                ans[5][1] = deladm * wm112;   simd_store(ans[5][1],&_ans[5][1]);

                ans[6][0] = adm * wm12;       simd_store(ans[6][0],&_ans[6][0]);
                ans[6][1] = deladm * wm122;   simd_store(ans[6][1],&_ans[6][1]);

                ans[7][0] = adm * wm21;       simd_store(ans[7][0],&_ans[7][0]);
                ans[7][1] = deladm * wm212;   simd_store(ans[7][1],&_ans[7][1]);

                ans[8][0] = adm * wm22;       simd_store(ans[8][0],&_ans[8][0]);
                ans[8][1] = deladm * wm222;   simd_store(ans[8][1],&_ans[8][1]);
				...
				for (i = 0; i < 4; i++)
				{
					se_write_buf[0][jse11[i] - 1] += _ans[1][0][i];
					se_write_buf[0][jse12[i] - 1] += _ans[2][0][i];
					se_write_buf[1][jse11[i] - 1] += _ans[3][0][i];
					se_write_buf[1][jse12[i] - 1] += _ans[4][0][i];
					se_write_buf[2][jse21[i] - 1] += _ans[5][0][i];
					se_write_buf[2][jse22[i] - 1] += _ans[6][0][i];
					se_write_buf[3][jse21[i] - 1] += _ans[7][0][i];
					se_write_buf[3][jse22[i] - 1] += _ans[8][0][i];
					
					dse_write_buf[0][jse11[i] - 1] += _ans[1][1][i];
					dse_write_buf[0][jse12[i] - 1] += _ans[2][1][i];
					dse_write_buf[1][jse11[i] - 1] += _ans[3][1][i];
					dse_write_buf[1][jse12[i] - 1] += _ans[4][1][i];
					dse_write_buf[2][jse21[i] - 1] += _ans[5][1][i];
					dse_write_buf[2][jse22[i] - 1] += _ans[6][1][i];
					dse_write_buf[3][jse21[i] - 1] += _ans[7][1][i];
					dse_write_buf[3][jse22[i] - 1] += _ans[8][1][i];
				}
            ...
            for (i = 0; i < _jl; i++)
			{
				thread_local_buff.se[kp2 + i] += se_write_buf[0][i];
				thread_local_buff.se[kp3 + i] += se_write_buf[1][i];
				thread_local_buff.se[im  + i] += se_write_buf[2][i];
				thread_local_buff.se[im1 + i] += se_write_buf[3][i];
				thread_local_buff.dse[kp2 + i] += dse_write_buf[0][i];
				thread_local_buff.dse[kp3 + i] += dse_write_buf[1][i];
				thread_local_buff.dse[im  + i] += dse_write_buf[2][i];
				thread_local_buff.dse[im1 + i] += dse_write_buf[3][i];
			}
        }
    }
}
