...
!*DeckYinxq: set_pebox
  ...
  subroutine set_pebox(im, jm, n, flagxcycle, halosize, lon, lat, mask, pebox, peleft, peright, peleftsend, perightsend, peleftrecv, perightrecv, peboxex)
    ...
    real(4) :: avenpx, avenpy, a, aa, avepoint,new_average,avenpy2
    integer, allocatable :: npx(:), npxstart(:), npxend(:),mask_water_only(:,:)
    integer :: i1, i2, i, n1, n2, ii, j1, j2, j, jj, nn,iii,jjj,all_procs
    integer :: nr, nl, nx, ny, nxmore, ny1, sum_ye
    integer :: isOverLoadX = 1
    integer :: isOverLoadY = 1
	real(4) :: weight=0.8
    ...
    integer :: nr_send, nr_recv, nl_send, nl_recv 
    integer :: continue_flag, inner_flag, iter_time

    !modified by Huang Chenghuan, 20170418
    integer, allocatable :: mask_walked(:)
    integer :: nnn, sum_col, sum_dst, sum_last, sum_cur, cur_row, diff_cur, diff_last, cur_id_in_col, row_start
    ...
    !    i = nx; nx = ny; ny = i
    !  endif
   ! write(*,*) myid,im,jm,n,flagxcycle,halosize
   ...
   ! --- Along x-direction.
    ...
    do ii = 1, nx
	  if (ii == 1) then
        new_average = avepoint
      else
        new_average = sum(mask_water_only(1:npxend(ii-1),:)) / float(all_procs)
      endif
	  
	   if(new_average <= avepoint) then
        isOverLoadX = 1
      else
        isOverLoadX = 0
      endif
       ...
       if(ii <= nxmore)then
         ...
         all_procs = all_procs + ny + 1
       else
		  avenpx = avepoint * ny
		  all_procs = all_procs + ny
       endif
      ...
      !write(*,*) ii,avepoint,new_average,avenpx
      if(ii == nx) then
                npxend(ii) = im
      else
              do i = i1+1, im
                n1 = sum(mask_water_only(i1:i-1, :))
                n2 = sum(mask_water_only(i1:i, :))
                if(n1 <= avenpx .and. n2 > avenpx)then
                        if(n2-avenpx < avenpx-n1) then
                                npxend(ii) = i
      ...
      sum_ye = sum_ye + sum(mask_water_only(npxstart(ii):npxend(ii),:))
     ! write(*,*) "column=",ii-1,'sum=',sum(mask_water_only(npxstart(ii):npxend(ii),:))
    enddo
   ! write(*,*) "x is ok"
  ! --- Along y-direction.
    nn = 0
    do ii = 1, nx
      ...
      i2 = npxend(ii)
      if(ii <= nxmore)then
        avenpy = sum(mask_water_only(i1:i2, :)) / (ny + 1); ny1 = ny + 1
      else
        avenpy = sum(mask_water_only(i1:i2, :)) / ny; ny1 = ny
      endif
      ...
      do jj = 1, ny1
        ...
        if(jj == ny1) then
                j2 = jm
         else if(ii==1 .and. jj ==1) then
	  	 do j = j1+1, jm
	   			  avenpy2=avenpy*weight
                 j2 = j
                 n1 = sum(mask_water_only(i1:i2, j1:j2-1))
                 n2 = sum(mask_water_only(i1:i2, j1:j2))
                  if(avenpy2 >= n1 .and. avenpy2 < n2) then
                        if(n2-avenpy> avenpy-n1) then
                                j2 = j2 - 1
                        endif
                        exit
                  endif
         enddo
                ...
                do j = j1+1, jm
                  ...
                  n1 = sum(mask_water_only(i1:i2, j1:j2-1))
                  n2 = sum(mask_water_only(i1:i2, j1:j2))
                  if(avenpy >= n1 .and. avenpy < n2) then
                        if(isOverLoadY.ne.1) then
                                j2 = j2 - 1
                        endif
                        exit
                  endif
		...
		!write(*,*) 'pro= ',nn-1, 'waternum=',sum(mask_water_only(i1:i2,j1:j2))
        
        new_average = sum(mask_water_only(i1:i2,1:j2)) / float(jj)

        if(new_average > avenpy) then
                isOverLoadY = 0
        else
                isOverLoadY = 1
        endif

        !write(*,*) "inner", sum(mask_water_only(i1:i2,j1:j2))
        sum_ye = sum_ye + sum(mask_water_only(i1:i2,j1:j2))
      enddo
      !write(*,*) "outer",sum_ye
    enddo
...
!    !check if there is any corner points
!    do while(continue_flag == 1)
!        inner_flag = 0
!        do nn = 1, n
!            i1 = pebox(2, nn)
!            i2 = pebox(3, nn)
...
!                endif
!                !check my right procs
!
!                if(j2 == pebox(4, i) - 1)then
!                    inner_flag = 1
!                    pebox(4, i) = pebox(4, i) + 1
!                    print *, "nn =", nn, "j2 =", j2, pebox(4, i)
!                endif
!                if(j1 == pebox(5, i) + 1)then
!                    inner_flag = 1
!                    pebox(5, i) = pebox(5, i) + 1
!                    print *, "nn =", nn, "j1 =", j1, pebox(5, i)
!                endif
!            end do
!
!
    ...
    do nn = 1, n
        ...
        do cur_row = 1, jm
            ...
            if(sum_cur >= sum_dst)then
                ...
                !print *, "sum_cur >= sum_dst, cur =", sum_cur, "last =", sum_last, "diff = ", diff_cur, diff_last
